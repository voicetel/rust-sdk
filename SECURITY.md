...
<https://github.com/voicetel/rust-sdk/security/advisories/new>).
...
- The SDK uses `reqwest` with `rustls` (TLS 1.2+) and a connection
  pool that reuses TLS sessions across requests. If you supply a
  custom `reqwest::Client`, you are responsible for matching that
  posture.
...
Out of scope: vulnerabilities in the `voicetel` crate caused by a
forked / vendored copy that has been modified.
